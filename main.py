...
# Создание одного конкретного поста
def create_post(VK_API, text):
    try:
        VK_API.wall.post(message=text)
        return True
    except Exception:
        print('ОШИБКА: Что-то пошло не так при создании поста...')
        return -1
...
def update_post(VK_API):
    ...
    try:
        VK_API.wall.edit(post_id=post[0], message=new_text)
...
def delete_post(VK_API):
    ...
    VK_API.wall.delete(post_id=post[0])
...
def get_posts(VK_API):
    posts_list_json = VK_API.wall.get()
...
def get_file_session():
    ...
    try:
        ...
        # Отсутствует(-ют) логин и/или пароль
        return None
...
def open_session():
    ...
    if saved_session is None or saved_session[0] is None or saved_session[1] is None:
        ...
        system('cls') # Очистка консоли для сокрытия логина
        password = input('Пароль: ')
        system('cls') # Очистка консоли для сокрытия пароля
    else:
        # Используем логин и пароль из файла
